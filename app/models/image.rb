class Image < ActiveRecord::Base
  ...
  has_attached_file :image, :styles => { :incident_block => "100x", :profile => "400x" }
  ...
  def extract_location
    if self.content_type == "image/jpeg"
      ...
      if exif && exif.gps_longitude && exif.gps_latitude && !exif.gps_longitude.nil? && !exif.gps_latitude.nil? && !exif.gps_longitude.empty? && !exif.gps_latitude.empty?
